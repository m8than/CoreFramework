...
class Template
{
    ...
    public static function dynamic_replacements($data)
    {
        ...
        {            
            ...
            for($i=0;$i<count($includes[0]);$i++)
            {
                ...
                $toreplace[$includes[0][$i]] = $includes[1][$i] . ' ' . $class_name . '::getCache("'.$includes[3][$i].'");';                
    ...
    public static function getCachePath($tpl_path)
    {
        ...
        return sprintf( ROOT . '%s/%s/%s_%s.php', self::$_cache_dir, $cur_route['controller'], $cur_route['method'], $view_hash);
    }
